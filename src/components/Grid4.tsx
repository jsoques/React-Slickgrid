...
export default class Grid4 extends Component<Props, State> {
  ...
  costDurationFormatter(
    ...
    const costText = this.isNullUndefinedOrEmpty(dataContext.cost)
      ? 'n/a'
      : Slicker.Utilities.formatNumber(
          dataContext.cost,
          0,
          2,
          false,
    ...
    if (
      !this.isNullUndefinedOrEmpty(dataContext.duration) &&
      dataContext.duration >= 0
    ) {
      durationText = `${dataContext.duration} ${
        dataContext.duration > 1 ? 'days' : 'day'
      }`;
    }
  ...
  setFrozenRowCount = () => {
    const frozenRowCountEl = document.getElementById(
      'frozenRowCount'
    ) as HTMLInputElement;
  ...
  setFrozenColumnsCount = () => {
    const frozenColumnCountEl = document.getElementById(
      'frozenColumnCount'
    ) as HTMLInputElement;
  ...
  executeCommand(_e, args) {
    ...
    switch (command) {
      ...
      case 'delete-row':
        if (
          confirm(
            `Do you really want to delete row (${args.row + 1}) with "${
              dataContext.title
            }"`
          )
        ) {
  ...
  componentDidMount() {
    ...
    this._bindingEventService.bind(
      gridContainerElm,
      'onvalidationerror',
      this.handleOnValidationError.bind(this)
    );
    this._bindingEventService.bind(
      gridContainerElm,
      'onitemdeleted',
      this.handleOnItemDeleted.bind(this)
    );
